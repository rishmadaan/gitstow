...
### Wave 1 — Correctness & safety (Opus) — NOT STARTED
- [ ] B1 deep-URL parsing guards (host-aware; GitHub/known hosts reject >2 path segments, strip `/tree|blob|pull|issues/...`)
- [ ] B3 atomic writes (temp + `os.replace`) + file locking on repos.yaml
- [ ] B4 `GIT_TERMINAL_PROMPT=0`, `LC_ALL=C` in `_run_git`; drop locale-dependent string matching where possible
- [ ] B2 fix or explicitly retire `config migrate-root` for the workspace era (+ DOC4)
- [ ] B5 route `repo freeze/unfreeze/tag/untag` through `resolve_repo` with `-w` support; distinguish ambiguous from missing (B13)
- [ ] S1 Origin/Host check middleware on all POST routes
- [ ] D1 test gate before PyPI publish
- [ ] B12 path-containment check in CLI `remove --delete`
- [ ] B11 workspace label validation
